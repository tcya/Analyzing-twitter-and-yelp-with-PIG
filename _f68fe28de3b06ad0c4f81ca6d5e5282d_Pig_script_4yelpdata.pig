...
Y = LOAD 'file:///usr/lib/hue/apps/search/examples/collections/solr_configs_yelp_demo/index_data.csv' USING CSVLoader() AS(business_id:chararray,cool,date,funny,id,stars:int,text:chararray,type,useful:int,user_id,name,full_address,latitude,longitude,neighborhoods,open,review_count,state);
...
Y_mall = GROUP Y_m2 ALL;
Y_m2avg = FOREACH Y_mall GENERATE AVG(Y_m2.avg_wtdstars);


Y_ratejoin = JOIN Y_rate2 BY business_id, Y_m2 BY business_idgroup;
Y_rateAll = GROUP Y_ratejoin ALL;
Y_rateavg = FOREACH Y_rateAll GENERATE AVG(Y_ratejoin.Y_rate2::wtd_stars);
